...
# Stored PR title prefix (branch slug uses a separate, deterministic title before the LLM runs).
...
def generate_pr_node(state: CrashState):
    ...
    Order: validate inputs → branch from main → apply patch → only then ask the LLM for PR title, body,
    and commit message (avoids token spend when apply fails) → commit scoped paths → push → open MR.
    ...
    try:
        ...
        # --- Step 1: feature branch from configured main (slug uses a fixed title; LLM runs only after apply). ---
        branch_slug_title = (state.get("pr_title") or "").strip() or "CrashLens fix"
        ...
        # --- Step 2: apply the fixer's unified diff to REPO_ROOT (normalization + git apply inside GitService). ---
        git.apply_unified_diff(patch)
        if crash_id:
            crash_store.set_pipeline_flags(crash_id, diff_applied=True)

        # --- Step 3: LLM generates MR title, body, and one-line commit message (skipped entirely if apply raised). ---
        llm = LLMService()
        pr_meta = parse_json(
            llm.call(
        ...
        # --- Step 4: normalize copy for storage and GitLab (prefix title, footer on body). ---
        ...
        # --- Step 5: commit only fix paths, push branch, open draft MR (title = commit subject convention). ---
        ...
        # Single catch: git apply, LLM JSON, commit, push, or GitLab API errors surface here for observability.
